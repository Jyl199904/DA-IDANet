    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        with torch.no_grad():
            ...
            for hr_img1, hr_img2, label in val_bar:
                ...
                # val_bar.set_desc f1, recall, precision, oa, iou = calculate_metrics(result, gt_value)
                #                 valing_results['IoU'] = iou
                #
                #                 val_bar.set_description(
                #                     desc=f'F1: {f1:.4f}, Recall: {recall:.4f}, Precision: {precision:.4f}, OA: {oa:.4f},IOU:{iou:.4f}')ription(
                #     desc='IoU: %.4f' % (  valing_results['IoU'],))
                # print(f'F1: {f1:.4f}, Recall: {recall:.4f}, Precision: {precision:.4f}, OA: {oa:.4f}')
