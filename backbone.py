...
class SELayer(nn.Module):
    ...
    def forward(self, x):
        b, c, _, _ = x.size()
        # 获取输入特征图x的维度信息，其中b表示批量大小，c表示通道数
        y = self.avg_pool(x).view(b, c)
        # 将输入特征图x进行全局平均池化，并将结果变换为形状为(batch_size, num_channels)的向量，以便输入到后续的全连接层。
        y = self.fc(y).view(b, c, 1, 1)
        # 将全局平均池化后的向量y输入到前面定义的全连接层序列中，得到一个形状为(batch_size, num_channels, 1, 1)的输出。这个输出包含了通道之间的权重信息。
        return x * y.expand_as(x)
        #将输入特征图x与通道权重y相乘，通过将通道权重在空间维度上进行扩展，以匹配输入特征图的形状。这个操作使得SE模块能够自适应地调整每个通道的重要性，从而增强特征表达能力。
...
#
#         # 添加深度可分离卷积
#         self.depthwise_conv = nn.Conv2d(inplanes, inplanes, kernel_size=3, stride=stride,
#                                         padding=dilation, dilation=dilation, groups=inplanes, bias=False)
#         self.pointwise_conv = nn.Conv2d(inplanes, planes, kernel_size=1, bias=False)
#
...
class Bottleneck(nn.Module):
    expansion = 4
    # 指示Bottleneck块中第三个卷积层的输出通道数是第一个卷积层输出通道数的4倍。
...
class PA(nn.Module):
    def __init__(self, inchan, out_chan):
        super().__init__()
    ...
    def forward(self, x):
        ...
        #将位置注意力权重与原始的降维特征图x0相乘，实现了自注意力机制，使得模型能够自适应地学习不同位置之间的权重。
...
class ResNet(nn.Module):
    ...
    def forward(self, input):
        ...
        out_s8, out_s4, out_s2 = self.pos_s8(x), self.pos_s4(low_level_feat3), self.pos_s2(low_level_feat2)
        return out_s8, out_s4, out_s2
