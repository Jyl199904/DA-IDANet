...
def calculate_metrics(predict, label):
    ...
    oa = calculate_overall_accuracy(tp, tn, fp, fn)
    iou = calculate_iou(predict,label)
    miou = calculate_miou(iou)
    return f1, recall, precision, oa, iou
...
class TestDatasetFromFolder(Dataset):
    def __init__(self, args, time1_dir, time2_dir, label_dir):
        ...
        #               os.path.splitext(name)[1] == item]
        # datalist = [name for name in os.listdir(time1_dir) if any(name.endswith(item) for item in self.args.suffix)]
        datalist = [name for name in os.listdir(time1_dir) if any(name.endswith(item) for item in IMAGES_FORMAT)]
